...
import { Wallet, ArrowRight, Zap, Database, X } from 'lucide-react'
                ...
                <div className="space-y-12">
                    <div className="space-y-4">
                        <div className="flex justify-between items-start">
                            <h2 className="text-4xl font-display font-medium text-black">Connect Wallet</h2>
                            <button onClick={onClose} className="text-gray-300 hover:text-black transition-colors">
                                <X size={24} />
                            </button>
                        </div>
                        <p className="text-vault-charcoal/50 font-medium">Select a provider to access the vault infrastructure.</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        {connectors.map((connector) => (
                            <button
                                ...
                                className="w-full flex items-center justify-between p-6 border border-black/[0.05] hover:border-black hover:bg-vault-off-white transition-all rounded-[4px] group relative overflow-hidden"
                            >
                                <div className="flex items-center gap-6 relative z-10">
                                    <div className="w-14 h-14 bg-black flex items-center justify-center rounded-[2px] group-hover:scale-105 transition-transform">
                                        {connector.name.toLowerCase().includes('metamask') ? (
                                            ...
                                            <div className="flex flex-col items-center gap-1">
                                                ...
                                                <span className="text-[6px] text-white font-bold uppercase">Core</span>
                                            </div>
                                        ) : (
                                    ...
                                    <div className="text-left">
                                        ...
                                        <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest leading-none">
                                            {connector.id === 'injected' ? 'MetaMask / Core / Injected' : 'External Provider'}
                                        </span>
                                    </div>
                                </div>
                                ...
                                {isPending && <div className="absolute inset-0 bg-black/5 animate-pulse" />}
                            </button>
                        ))}
                    </div>
